...
class FeaturesConfig:
    ...
    parallel_enhanced_alignment: ConfigValue[bool] = field(default_factory=lambda: ConfigValue(
        default=True,
        env_var="FACE_SEQ_PARALLEL_ENHANCED_ALIGNMENT",
        description="Run silence detection and forced alignment concurrently to reduce total latency"
    ))
...
def configure_app(app):
    ...
    app.config['DEBUG'] = config.server.debug.get()
    app.config['MAX_CONTENT_LENGTH'] = config.limits.max_upload_size.get()
    
    # Ensure required directories exist
    config.paths.ensure_directories_exist()
    
    # Configure upload folders
    app.config['UPLOAD_FOLDER'] = str(config.paths.upload_folder.get())
    
    # Set allowed extensions
    app.config['ALLOWED_AUDIO_EXTENSIONS'] = config.limits.allowed_audio_extensions.get()
    
    # Configure logging
    log_level_str = config.logging.log_level.get().upper()
    ...
    if config.logging.log_to_file.get():
        log_file_path = config.paths.logs_folder.get() / config.logging.log_file.get()
