...
def ensure_whisperx_safe_globals() -> None:
    ...
    try:
        ...
        # Add more OmegaConf internal classes that WhisperX might use
        try:
            listconfig_mod = importlib.import_module("omegaconf.listconfig")
            ListConfigClass = getattr(listconfig_mod, "ListConfig", None)
        except Exception:
            ListConfigClass = None
        
        try:
            dictconfig_mod = importlib.import_module("omegaconf.dictconfig")
            DictConfigClass = getattr(dictconfig_mod, "DictConfig", None)
        except Exception:
            DictConfigClass = None
            
        classes: Iterable[Any] = [c for c in [
            ListConfig, DictConfig, OmegaConf, ContainerMetadata, Node,
            ListConfigClass, DictConfigClass
        ] if c is not None]
        
        if classes:
            try:
                torch.serialization.add_safe_globals(list(classes))  # type: ignore[attr-defined]
                print(f"[PYTORCH] Registered {len(classes)} OmegaConf classes as safe globals")
...
def patched_load_context() -> Iterator[None]:
    ...
    # CRITICAL: Ensure safe globals are registered BEFORE patching
    ensure_whisperx_safe_globals()
