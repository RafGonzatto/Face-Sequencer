...
from app.core.subtitles.subtitle_engine import SubtitleSegment, SubtitleStyle, PlatformPreset
...
class SubtitleRenderer:
    ...
    def _verify_ffmpeg(self):
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"[CHECK] FFmpeg available: {self.ffmpeg_path}")
            else:
                raise RuntimeError("FFmpeg not responding correctly")
        except Exception as e:
            print(f"[X] FFmpeg not found at {self.ffmpeg_path}: {e}")
    ...
    def render_subtitles_on_video(
        ...
        print(f"[CLAPPER] Rendering {len(subtitles)} subtitles onto video...")
        ...
        try:
            ...
            if success and os.path.exists(output_video_path):
                ...
                print(f"[CHECK] Video with subtitles rendered successfully ({file_size:.1f} MB)")
                return output_video_path
            else:
                raise RuntimeError("FFmpeg rendering failed or output file not created")
                
        except Exception as e:
            print(f"[X] Subtitle rendering failed: {e}")
            raise
        finally:
    ...
    def _generate_ass_subtitles(
        ...
        print(f"[NOTE] Generated ASS subtitle file: {ass_file}")
        return ass_file
...
"""
    ...
    def _build_ffmpeg_command(
        ...
        cmd = [
            self.ffmpeg_path,
            "-i", input_video,
            # Escape Windows path for FFmpeg filter; prefer basename when possible
            # to avoid drive letter colon issues when cwd matches the temp dir.
            "-vf", (
                f"ass={os.path.basename(ass_file)}" if os.path.dirname(ass_file) == os.getcwd() else (
                    f"ass={ass_file.replace('\\\\', r'\\').replace(':', r'\:')}" if os.name == 'nt' else f"ass={ass_file}"
                )
            ),
            "-c:v", "libx264",
    ...
    def _execute_ffmpeg_with_progress(
        ...
        try:
            ...
            if process.returncode == 0:
                print("[CHECK] FFmpeg rendering completed successfully")
                return True
            else:
                print(f"[X] FFmpeg failed with return code {process.returncode}")
    ...
    def _track_ffmpeg_progress(self, process: subprocess.Popen, callback: callable):
        ...
        try:
            ...
            print(f"[WARNING] Progress tracking error: {e}")
    ...
    def create_subtitle_preview(
        ...
        try:
            ...
            print(f"[CHECK] Subtitle preview created: {output_path}")
            return output_path
        except subprocess.CalledProcessError as e:
            print(f"[X] Preview generation failed: {e}")
            raise
    ...
    def extract_audio_from_video(self, video_path: str, output_audio_path: str) -> str:
        ...
        try:
            ...
            print(f"[CHECK] Audio extracted: {output_audio_path}")
            return output_audio_path
        except subprocess.CalledProcessError as e:
            print(f"[X] Audio extraction failed: {e}")
            raise
...
def test_subtitle_renderer():
    ...
    try:
        ...
        from app.core.subtitles.subtitle_engine import SubtitleSegment, SubtitleStyle
        ...
        print(f"[CHECK] ASS file generated: {ass_file}")
        ...
        print(f"[CHECK] Preview created: {preview_path}")
        
        print("[CHECK] Subtitle Renderer test completed")
