...
class PreciseFrameSynchronizer:
    ...
    def __init__(self, fps: float = 30.0, audio_sample_rate: int = 16000):
        ...
        print(f"[CLAPPER] FrameSynchronizer initialized: {fps} FPS, {self.frame_duration*1000:.1f}ms per frame")
    ...
    def create_animation_timeline(self, 
        ...
        print(f"[TARGET] Creating timeline for {len(word_timings)} words over {total_duration:.2f}s")
        ...
        print(f"[CHECK] Timeline created with sub-frame precision")
        print(f"[CHART] Timeline: {len([t for t in timeline if t.word != 'PAUSE'])} words, {len([t for t in timeline if t.word == 'PAUSE'])} pauses")
        return timeline
    ...
    def generate_frame_sequence(self, 
        ...
        print(f"[CLAPPER] Generating {total_frames} frames at {self.fps} FPS")
        print(f"[CHART] Precise timing - Audio: {precise_duration:.6f}s, Video: {calculated_video_duration:.6f}s")
        print(f"[CHART] Timing difference: {abs(calculated_video_duration - precise_duration)*1000:.1f}ms")
        ...
        if frame_states:
            ...
            if timing_error > 50:  # More than 50ms error
                print(f"[WARNING]  Warning: Significant timing error: {timing_error:.1f}ms")
            else:
                print(f"[CHECK] Timing accurate within {timing_error:.1f}ms")
        
        print(f"[CHECK] Generated {len(frame_states)} frame states")
        return frame_states
    ...
    def export_timeline_json(self, timeline: List[WordTiming], output_path: str):
        ...
        print(f"[FOLDER] Timeline exported to {output_path}")
...
if __name__ == "__main__":
    ...
    print(f"\n[CLAPPER] Generated {len(frame_states)} frame states")
    print("Sample frames:")
