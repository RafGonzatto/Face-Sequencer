...
class EnhancedSilenceDetector:
    ...
    def detect_silence_segments(self, 
        ...
        if adaptive_thresholds:
            ...
            print(f"[CHART] Adaptive thresholds - Energy: {self.energy_threshold:.4f}, ZCR: {self.zcr_threshold:.4f}")
        else:
        ...
        print(f"[CHECK] Detected {len(silence_segments)} silence segments")
    ...
    def visualize_detection(self, audio_data: np.ndarray, segments: List[SilenceSegment], 
        ...
        try:
            ...
            if output_path:
                ...
                print(f"[CHART] Visualization saved to {output_path}")
            else:
                plt.show()
