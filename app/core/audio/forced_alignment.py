...
class ForcedAligner:
    ...
    def __init__(self, device: str = "auto", language: str = "pt"):
        ...
        print(f"[TARGET] Initializing ForcedAligner for language: {language}")
    ...
    def _setup_device(self, device: str) -> str:
        ...
        if device == "auto":
            if torch.cuda.is_available():
                ...
                device = "cpu"
                print("[COMPUTER] Using CPU for alignment")
        return device
    ...
    def _load_wav2vec2_model(self) -> Tuple[object, object]:
        ...
        try:
            ...
            print(f"[X] Transformers not available: {e}")
            return None, None
        ...
        for name in candidates:
            try:
                print(f"[PACKAGE] Loading Wav2Vec2 model: {name}")
                ...
                try:
                    ...
                    try:
                        ...
                        last_err = e2
                        print(f"[WARNING] Retry without safetensors failed for {name}: {e2}")
                        continue
                ...
                last_err = e
                print(f"[WARNING] Could not load Wav2Vec2 '{name}': {e}")

        print(f"[X] Failed to load any Wav2Vec2 model: {last_err}")
        print("[REFRESH] Falling back to duration-based alignment or Whisper where available")
        return None, None
    ...
    def _load_whisper_model(self) -> object:
        ...
        try:
            ...
            print(f"[PACKAGE] Loading Whisper model: {model_size}")
            ...
            print(f"[X] Failed to load Whisper model: {e}")
            return None
    ...
    def align_text_to_audio(self, 
        ...
        """
        print(f"[TARGET] Starting forced alignment for {len(text)} characters of text")
        print(f"[CHART] Audio: {len(audio)/sample_rate:.2f}s at {sample_rate}Hz")
        ...
        try:
            ...
            print(f"[CHECK] Alignment completed: {len(result.tokens)} tokens, avg confidence: {result.avg_confidence:.3f}")
            return result
            
        except Exception as e:
            print(f"[X] Alignment failed with {method}: {e}")
            # Fallback to simple timing estimation (never raises)
            print("[REFRESH] Falling back to duration-based timing")
    ...
    def _align_with_whisper(self, 
        ...
        if sample_rate != 16000:
            try:
                from app.core.audio.audio_utils_safe import import_librosa
    ...
    def _fallback_alignment(self, 
        ...
        print("[WARNING] Using fallback duration-based alignment")
    ...
    def export_alignment(self, result: AlignmentResult, output_path: str):
        ...
        print(f"[FOLDER] Alignment exported to {output_path}")
...
if __name__ == "__main__":
    try:
        ...
        print("[X] librosa not available for testing")
        exit(1)
