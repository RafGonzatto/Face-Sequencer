...
def process_elevenlabs_audio(input_file=None, output_file=None, plot=False, verbose=True):
    ...
    if input_file is None:
        ...
        if not elevenlabs_files:
            if verbose:
                print("[X] Error: No ElevenLabs audio files found in the current directory")
            return False
    ...
    if verbose:
        ...
        print(f"[FOLDER] Output will be saved to: {output_file}")
    
    try:
        # Load audio with original sample rate
        if verbose:
            print("[CHART] Loading audio...")
        ...
        if plot:
            ...
            if verbose:
                print(f"[CHART] Generating audio comparison plot: {plot_filename}")
            ...
            if verbose:
                print(f"[CHART] Plot saved to {plot_filename}")
        
        if verbose:
            print("[CHECK] Audio enhancement complete!")
        
        return True
        
    except Exception as e:
        import traceback
        print(f"[X] Error during audio processing: {str(e)}")
        traceback.print_exc()
        return False
