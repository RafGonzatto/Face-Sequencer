...
from app.core.export.social_media_exporter import SocialMediaExporter, SocialMediaPreset
...
def get_social_media_presets():
    ...
    try:
        ...
        return error_response("Failed to retrieve presets", status=500)
...
def get_preset_details(preset_name):
    ...
    try:
        ...
        if not preset:
            return error_response(f"Preset not found: {preset_name}", status=404)
        ...
        return error_response("Failed to retrieve preset details", status=500)
...
def get_preset_recommendations():
    ...
    try:
        ...
        return error_response("Failed to generate recommendations", status=500)
...
async def export_video_with_subtitles():
    ...
    try:
        ...
        if not os.path.isabs(input_video_path):
            # Prefer resolving relative to the project root (one level above app/)
            app_root = os.path.abspath(current_app.root_path)  # .../im/app
            project_root = os.path.abspath(os.path.join(app_root, os.pardir))  # .../im

            candidates = []  # in order of preference
            # If client sent something like "uploads/audio/filename.ext", resolve from project root
            candidates.append(os.path.join(project_root, input_video_path))
            # Also try relative to app/ (legacy behavior)
            candidates.append(os.path.join(app_root, input_video_path))

            # Pick the first existing candidate; else keep the first (project_root) for error message clarity
            resolved = candidates[0]
            for cand in candidates:
                if os.path.exists(cand):
                    resolved = cand
                    break
            input_video_path = resolved
        
        # Validate input video exists
        if not os.path.exists(input_video_path):
            return error_response(f"Input video file not found: {input_video_path}", status=404)
        
        # Convert subtitle data to SubtitleSegment objects
        subtitle_segments = []
        for seg_data in subtitle_segments_data:
            try:
                style_overrides = seg_data.get('style_overrides') or {}
                # Forward karaoke info within style_overrides to avoid changing dataclass
                if 'karaoke' in seg_data and isinstance(style_overrides, dict):
                    style_overrides = dict(style_overrides)
                    style_overrides['karaoke'] = seg_data['karaoke']
                ...
                return error_response(f"Invalid subtitle segment data: {e}", status=400)
        
        if not subtitle_segments:
            return error_response("No valid subtitle segments provided", status=400)
        ...
        # Convert output directory to absolute path if needed (prefer project root)
        if not os.path.isabs(output_dir):
            app_root = os.path.abspath(current_app.root_path)
            project_root = os.path.abspath(os.path.join(app_root, os.pardir))
            # Prefer writing to project-level exports/
        ...
        if export_result['success']:
            ...
            return error_response(export_result['error'], status=500)
        
    except Exception as e:
        logger.error(f"Video export with subtitles failed: {e}")
        return error_response("Export failed", status=500)
...
async def batch_export_videos():
    ...
    try:
        ...
        if not export_jobs:
            return error_response("No export jobs provided", status=400)
        ...
        for i, job_data in enumerate(export_jobs):
            ...
            if missing_fields:
                return error_response(f"Job {i}: Missing fields: {', '.join(missing_fields)}", status=400)
        ...
        return error_response("Batch export failed", status=500)
...
def get_export_job_status(job_id):
    ...
    try:
        ...
        return error_response("Failed to get job status", status=500)

@phase4_export_bp.route('/download/<job_id>', methods=['GET'])
def download_exported_video(job_id):
    """Stream the exported video file as an attachment for a completed job."""
    try:
        # Ensure job exists and completed
        job_status = social_exporter.get_job_status(job_id)
        if not job_status:
            return error_response("Job not found", status=404)
        if job_status.get('status') != 'completed':
            return error_response(f"Job not completed. Status: {job_status.get('status')}", status=400)

        # Access the underlying job to get the output path
        job = social_exporter.active_jobs.get(job_id)
        if not job:
            return error_response("Job data not available", status=404)
        output_path = getattr(job, 'output_path', None)
        if not output_path or not os.path.exists(output_path):
            return error_response("Output file not found", status=404)

        filename = os.path.basename(output_path)
        return send_file(output_path, as_attachment=True, download_name=filename, mimetype='video/mp4')

    except Exception as e:
        logger.error(f"Failed to serve download for {job_id}: {e}")
        return error_response("Download failed", status=500)
...
def optimize_subtitles_for_platform():
    ...
    try:
        ...
        if not subtitle_segments_data:
            return error_response("No subtitle segments provided", status=400)
        ...
        if not preset:
            return error_response(f"No preset found for platform: {platform}", status=400)
        ...
        return error_response("Platform optimization failed", status=500)
...
def validate_video_duration():
    ...
    try:
        ...
        if duration <= 0:
            return error_response("Invalid duration", status=400)
        ...
        return error_response("Duration validation failed", status=500)
...
def get_export_statistics():
    ...
    try:
        ...
        return error_response("Failed to retrieve statistics", status=500)

@phase4_export_bp.route('/preview-frame', methods=['POST'])
async def render_preview_frame():
    """Render a pixel-perfect preview frame (PNG) using the same pipeline as export."""
    try:
        data = request.get_json() or {}
        input_video_path = data.get('input_video_path')
        subtitle_segments_data = data.get('subtitle_segments') or []
        time_sec = float(data.get('time', 0))
        preset_name = data.get('preset_name', 'instagram_reel')
        custom_settings = data.get('custom_settings', {})

        if not input_video_path or not subtitle_segments_data:
            return error_response("Missing input_video_path or subtitle_segments", status=400)

        # Resolve relative path to project root first (consistent with export)
        if not os.path.isabs(input_video_path):
            app_root = os.path.abspath(current_app.root_path)
            project_root = os.path.abspath(os.path.join(app_root, os.pardir))
            candidates = [
                os.path.join(project_root, input_video_path),
                os.path.join(app_root, input_video_path),
            ]
            for cand in candidates:
                if os.path.exists(cand):
                    input_video_path = cand
                    break
        ...
        result = await social_exporter.render_preview_frame(
            input_video_path=input_video_path,
        ...
        if not result.get('success'):
            return error_response(result.get('error', 'Preview failed'), status=500)

        # Stream the PNG
        png_path = result['image_path']
        filename = os.path.basename(png_path)
        return send_file(png_path, as_attachment=False, download_name=filename, mimetype='image/png')
    except Exception as e:
        logger.error(f"Preview frame failed: {e}")
        return error_response("Preview failed", status=500)
...
def cleanup_old_jobs():
    ...
    try:
        ...
        return error_response("Cleanup failed", status=500)
