...
class VideoEditorCore extends EventTarget {
  constructor(appRef) {
    ...
    if (!this.app) {
      ...
      console.warn('[VideoEditorCore] No host app supplied; using internal no-op logger app');
    } else {
    ...
    // Safe wrappers
    this.safeStatus = (msg) => { try { this.app?.showStatus?.(msg); } catch(e){ console.log('[safeStatus]', msg); } };
    this.safeError = (msg) => { try { this.app?.showError?.(msg); } catch(e){ console.error('[safeError]', msg); } };
    this.safeToast = (msg, opts) => { try { this.app?.errorToasts?.show?.(msg, opts); } catch(e){ console.log('[safeToast]', msg, opts||''); } };
    ...
    setTimeout(() => this.attachUndoRedoShortcuts && this.attachUndoRedoShortcuts(), 0);
    ...
    this.exportVideoWithSubtitles = document.getElementById('exportVideoWithSubtitles');
    ...
    this.faceAnimationInterface = document.getElementById('faceAnimationInterface');
    ...
    if (!this.precisionModeSelect) {
      const hostControls = document.querySelector('.video-editor-controls, .player-controls, .video-controls');
      if (hostControls) {
    ...
    if (!this.effectsPanel) {
      const container = document.querySelector('#subtitleStylePanel, .subtitle-style-panel, .style-controls');
      if (container) {
        const panel = document.createElement('div');
        panel.id = 'subtitleEffectsPanel';
        Object.assign(panel.style, { marginTop: '12px', padding: '8px', border: '1px solid #333', borderRadius: '4px', background: '#1e1e1e' });
