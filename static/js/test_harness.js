...
(function () {
  ...
  if (!window.videoEditor) {
    window.videoEditor = {
      _earlyStub: true,
      alignAudioWithText: function (blob, text) {
        console.log(
          "[test-harness earlyStub] alignAudioWithText invoked (will be replaced later)"
        );
        return Promise.resolve({ stub: true });
      },
      generateSubtitles: function () {
        console.log(
          "[test-harness earlyStub] generateSubtitles called before module init"
        );
      },
    };
  }
  ...
  function ensurePartialPanel(initial) {
    ...
    panel.style.cssText =
      "position:fixed;bottom:6px;right:6px;background:#1f2937;color:#fff;padding:6px 8px;font:11px/1.3 system-ui;border:1px solid #374151;border-radius:4px;z-index:50000;";
  ...
  window.__synthesizeTestAlignment = function () {
    try {
      if (!document.querySelector(".partial-transcript-panel")) {
        ...
        p.style.cssText =
          "position:fixed;bottom:10px;right:10px;background:#111827;color:#fff;padding:8px 10px;font:12px/1.4 system-ui;border:1px solid #374151;border-radius:6px;z-index:50000;max-width:220px;";
