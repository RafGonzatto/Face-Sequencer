...
(function () {
  ...
  const noSmoke =
    window.location.search.includes("NO_SMOKE=1") ||
    window.location.hash.includes("NO_SMOKE");
  if (noSmoke) {
    console.log("[smoke] Skipped due to NO_SMOKE flag");
    return;
  }
  ...
  try {
    console.log(
      "[smoke] Starting VideoEditorModule headless instantiation test"
    );
    ...
    try {
      mod.updateGenerateButton && mod.updateGenerateButton();
    } catch (e) {
      console.warn("[smoke] updateGenerateButton error", e);
    }
    ...
    try {
      mod.previewWithSubtitles();
    } catch (e) {
      console.error("[smoke] previewWithSubtitles threw", e);
    }

    console.log(
      "[smoke] VideoEditorModule basic methods executed without fatal errors"
    );
  } catch (err) {
    console.error(
      "[smoke] Failure constructing VideoEditorModule without host app",
      err
    );
  }
})();
