...
(function () {
  ...
  const enabled =
    window.__FRONTEND_TEST_MODE__ ||
    /[?&]UNIT_TESTS=1/.test(q) ||
    h.includes("unit-tests");
  if (!enabled) return;
  ...
  const assert = (name, cond, info) => {
    ...
    if (!cond) console.error("[test fail]", name, info || "");
  ...
  function summary() {
    ...
    panel.style.cssText =
      "position:fixed;bottom:10px;right:10px;background:#111;color:#eee;font:12px monospace;padding:10px 14px;border:1px solid #333;border-radius:6px;z-index:99999;max-width:300px;";
  ...
  try {
    ...
    editor.subtitles = [
      { id: "s1", text: "Hello world line", start_ms: 0, end_ms: 1500 },
    ];
    editor._undoStack = [];
    editor._redoStack = [];
    editor._maxHistory = 10;
    if (editor._pushHistory) editor._pushHistory();
    assert(
      "undo stack after push",
      editor._undoStack.length === 1,
      "length=" + editor._undoStack.length
    );

    // Apply a preset (if styles module loaded)
    if (window.FaceSeqEditorModules?.styles) {
      try {
        editor.applyPreset("tiktok");
        assert(
          "preset application updates currentPreset",
          editor.currentPreset === "tiktok"
        );
      } catch (e) {
        assert("preset application did not throw", false, e.message);
      }
    }

    // Export methods presence (non-executed to avoid network)
    assert(
      "exportVideo present (export module)",
      typeof editor.exportVideo === "function"
    );
    ...
    if (window.FaceSeqEditorModules?.timeline) {
      ...
      const formatted = editor._formatSubtitleLines(
  ...
  // Basic DOM presence tests for restored minimal controls (non-fatal if missing)
  const idsToCheck = [
    "frameDuration",
    "pauseDuration",
    "fps",
