...
(function(){
  ...
  proto._applyOverlayPosition = function(){
    ...
    if (coord) coord.textContent = `${this.overlayPosition.xPercent.toFixed(1)}%, ${this.overlayPosition.yPercent.toFixed(1)}%`;
  };

  proto.enableOverlayDragging = function(overlay, handle){
    if (!overlay) return;
    let dragging = false;
    let startX=0,startY=0,startXP=0,startYP=0;
    const down = (e)=>{
      ...
      startXP = ((rect.left + rect.width/2 - videoRect.left) / videoRect.width) * 100;
      startYP = ((rect.top + rect.height/2 - videoRect.top) / videoRect.height) * 100;
    ...
    const up = ()=>{
      if (dragging){
        ...
        try { localStorage.setItem('subtitleOverlayPosition', JSON.stringify(this.overlayPosition)); } catch(_){ }
      }
    };
  ...
  proto._computeDynamicVerticalPosition = function(style){
    const controls = document.querySelector('.video-controls, .player-controls');
    if (controls && this.videoPreview){
      const videoRect = this.videoPreview.getBoundingClientRect();
      const controlsRect = controls.getBoundingClientRect();
      if (controlsRect.top < videoRect.bottom && controlsRect.top > videoRect.bottom - videoRect.height * 0.25){
        return 'top';
      }
    }
